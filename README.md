...
- 🩺 **诊断 Agent** — 一键诊断模式：症状输入自动附带只读体检快照（CPU / 内存 / 磁盘 / 端口 / 近期错误日志），按内置场景包（CPU 高 / 磁盘满 / OOM / 服务异常 / 端口不通 / 容器重启循环等 9 个）决策树排查，结论按「现象 / 根因 / 证据 / 建议 / 风险」输出；诊断会话可经 AI 提炼沉淀为新场景（SKILL.md），支持场景库管理
