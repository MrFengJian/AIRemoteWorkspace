...
- 🧑‍💻 **数字员工（运维专家）** — 角色化专家系统：内置 K8s 运维 / K8s 应用开发 / Docker / Linux 系统 / 数据库 / SRE 诊断 6 类专家，每位专家默认绑定对应技能包（如 K8s 运维绑定 Pod 排障决策树、Linux 绑定服务排障与定时任务速查、DBA 绑定 MySQL 陷阱速查），也可自定义人设（身份档案 + 人设指令 + 工具白名单 + 绑定技能 + 默认模型 + 开场白）；专家可连同绑定技能整体导出为 zip 包、再导入恢复（导入为新建自定义专家，防 zip-slip 与体积护栏），按业界数字员工最佳实践设计，人设不可越过审批安全边界
