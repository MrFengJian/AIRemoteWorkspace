...
- 🧑‍💻 **数字员工（运维专家）** — 按业界 Agent 目录布局组织的专家系统：每位专家是一个目录（manifest.json 身份卡 + SOUL.md 人设核心 + HEARTBEAT.md 值守指引 + skills/ 私有技能包），内置 K8s 运维 / K8s 应用开发 / Docker / Linux 系统 / 数据库 / SRE 诊断 6 类专家与**默认专家「通用助手」**（自带欢迎词与推荐问题）；专家私有技能不进公共技能列表，与公共技能重名时该专家会话内私有版优先；专家可整体导出为 zip 包、再导入恢复（新建自定义专家，防 zip-slip 护栏）；人设不可越过审批安全边界
- 🩺 **诊断 Agent** — 诊断模式即内置「SRE 诊断专家」：症状输入自动附带只读体检快照（CPU / 内存 / 磁盘 / 端口 / 近期错误日志），按内置场景包（CPU 高 / 磁盘满 / OOM / 服务异常 / 端口不通 / 容器重启循环等 14 个，含取自 SkillHub 社区的 MIT-0 技能改编包）决策树排查，结论按「现象 / 根因 / 证据 / 建议 / 风险」输出；技能支持目录形态（可附带脚本与参考文档，Agent 经 skill 工具按路径读取附件），诊断会话可经 AI 提炼沉淀为新场景（SKILL.md），支持场景库管理
