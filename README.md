...
- 🧑‍💻 **数字员工（运维专家）** — 角色化专家系统：内置 K8s 运维 / K8s 应用开发 / Docker / Linux 系统 / 数据库 / SRE 诊断 6 类专家，也可自定义人设（身份档案 + 人设指令 + 工具白名单 + 绑定技能 + 默认模型 + 开场白），按业界数字员工最佳实践设计，人设不可越过审批安全边界
- 🩺 **诊断 Agent** — 诊断模式即内置「SRE 诊断专家」：症状输入自动附带只读体检快照（CPU / 内存 / 磁盘 / 端口 / 近期错误日志），按内置场景包（CPU 高 / 磁盘满 / OOM / 服务异常 / 端口不通 / 容器重启循环等 9 个）决策树排查，结论按「现象 / 根因 / 证据 / 建议 / 风险」输出；诊断会话可经 AI 提炼沉淀为新场景（SKILL.md），支持场景库管理
