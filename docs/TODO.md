...
- [~] 会话日志（Session Logging）— 会话右键菜单版已上线：菜单「日志」启停（勾选态）+ tee 落盘 + 打开日志目录
  ...
  - [x] 全局开关 + 时间戳前缀（设置 → 高级，行级 HH:MM:SS 前缀，跨行缓冲安全）
  - [x] 设置页管理入口（高级 → 会话日志卡片）
  - [ ] 按主机覆盖
...
- [x] 同步键入（broadcast input）：快速命令栏新增开关（Radio 按钮），开启后焦点窗格键入实时镜像到目标窗格（写入走 sendPayloadToTabs，源窗格自然接收不重复）
- [ ] Zmodem（rz/sz）：识别 ZRQINIT/ZRINIT 自动拉起，传输进度 UI
- [x] 登录脚本（expect 序列）：主机级「等待→发送」步骤表，会话打开后按顺序自动执行（子串匹配 + 15s 步骤超时，超时静默中止不干扰会话）
- [x] 会话恢复：重启后按上次打开的标签恢复（SSH 走密码库免密重连；已删除主机跳过；分屏布局暂不恢复）
- [x] scrollback 行数进设置（全局默认，新建窗格生效、现有窗格热更）
- [x] 本地终端默认命令行：按 OS 检测可用 shell（Windows: pwsh/PowerShell/CMD/Git Bash + WSL 按实例展开——`wsl -l -v` 解析实例名与版本，无实例或不可用则不出现；Unix: $SHELL + /etc/shells），设置页选默认，+ 号旁下拉按会话选择（仅一个时不显示）
### P2 — 覆盖面（按反馈排期）
...
- [x] 系统通知（窗口未聚焦时会话结束提醒；Web Notification API，设置可关）
- [x] 远程文件右键本地编辑、保存自动回传（临时副本 + mtime 监视自动上传，编辑同一文件复用同一副本）

---

## Phase 9 — 运维专家（角色系统）

> 背景与方案见 [ROADMAP.md](./ROADMAP.md) Phase 9。按业界运维专家系统最佳实践建模；诊断模式统一为内置 SRE 诊断专家。
...
- [x] 专家 zip 包导出/导入（`application/expert_transfer.go` + Wails `ExportExpert`/`ImportExpert`）
  ...
  - [x] 前端：设置 → 运维专家 增加 导入/导出 按钮（Dialogs 选路径 + 确认弹窗 + toast）；场景库附件数徽标；i18n 双语
...
- [x] 前端
  ...
  - [x] 诊断入口：⚡ 按钮 → 诊断对话框 → 选中诊断专家发送（AutoSnapshot 注入快照），场景种子流程不变（**该入口已退役**，见「诊断模式退役」条目）
