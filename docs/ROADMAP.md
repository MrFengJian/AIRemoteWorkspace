...
`FaultReportService`（sqlite `fault_reports` 表）+ Agent 面板「生成故障报告」入口：基于当前会话记录的一次性 LLM 提炼（`Runtime.DistillFaultReport` 提示词要求只输出 JSON：标题 ≤40 字、severity 三级、body 按 SRE 五段格式 现象/根因/证据/处置/预防），用户预览编辑后保存，附着在主机上作为可回溯资产（保留来源会话 ID）。独立「故障报告」导航页支持按主机 / 关键字（标题+正文）/ 级别 / 状态过滤，报告沿「未解决 → 观察中 → 已解决」生命周期跟踪。
