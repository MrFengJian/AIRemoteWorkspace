---
name: mysql-triage
description: MySQL 陷阱与生产实践速查：字符集、索引设计、事务与锁、UPSERT、连接管理、复制与性能优化
---
# MySQL 陷阱与生产实践
...
> 来源：改编自 skillhub.cn 收录的 mysql（clawhub @ivangdavila），许可证 MIT-0。
> 原技能的 indexes/transactions/queries/production 分册未随附，本包仅保留单文件速查内容；
> DDL / KILL / 配置变更等操作在本产品中需用户确认后执行。
