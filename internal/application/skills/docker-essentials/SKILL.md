...
description: Docker 命令与工作流速查：容器生命周期、日志与调试、镜像构建、Compose、网络、卷与系统清理
---
# Docker Essentials
...
> 来源：改编自 skillhub.cn 收录的 docker-essentials（clawhub @arnarsson），许可证 MIT-0。
> 变更类命令（run/stop/rm/prune/down 等）在本产品中需用户确认后执行。
