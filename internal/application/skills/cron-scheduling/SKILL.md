...
description: cron 与 systemd timer 定时任务速查：语法、crontab 管理、时区陷阱、任务未执行的排查与防重叠
---
# Cron & Scheduling
...
This pack bundles a ready-to-use wrapper script at `scripts/cron-wrapper.sh`
(read it via the skill tool's `path` argument): it logs start/success/failure
with UTC timestamps and per-job log files under `/var/log/cron-jobs/`, and
alerts by mail on failure. Wire it into a crontab like:
...
> 来源：改编自 skillhub.cn 收录的 cron-scheduling（clawhub @gitgoodordietrying），许可证 MIT-0。
> 修改 crontab、创建/启停 timer、删除任务等变更操作在本产品中需用户确认后执行。
